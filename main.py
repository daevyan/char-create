...
class ConsoleInt(object):
    def __init__(self):
        self.answer_range = range(20, 150)
        self.range_dict = {
            "a kid": range(5, 11),
            "a teen": range(11, 18),
    ...
    def console_int(self, answer, data_type):  # Check if answer in range
        def check_if_int():
        ...
        def check_if_in_range(int_answer):
            if int_answer in self.answer_range:
                if self.range_dict is not None:
                    get_range_name(int_answer, self.range_dict)
                else:
                    return int_answer
            else:
                print "This value is not in range. Range is %s to %s." \
                      % (self.answer_range[0], self.answer_range[-1])
        ...
        if is_int and self.answer_range is not None:
...
thing = b.console_int(le_answer, "number")
print thing
